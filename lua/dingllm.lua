...
function M.make_anthropic_spec_curl_args(opts, prompt, system_prompt)
	local url = opts.url
	local api_key = opts.api_key or (opts.api_key_name and get_api_key(opts.api_key_name))
	local data = {
	...
	local args =
		{ "-N", "-X", "POST", "-H", "Content-Type: application/json", "-d", vim.json.encode(data), "--max-time", "30" }
	if api_key then
		table.insert(args, "-H")
...
function M.handle_anthropic_spec_data(data_stream, event_state)
	if event_state == "content_block_delta" then
		...
		if success then
			print("Debug: Decoded JSON: " .. vim.inspect(json))
...
function M.invoke_llm_and_stream_into_editor(opts, make_curl_args_fn, handle_data_fn)
	if not opts.api_key and not (opts.api_key_name and vim.fn.getenv(opts.api_key_name)) then
		print("Error: API key is missing. Please set the ANTHROPIC_API_KEY environment variable.")
		return
	end
